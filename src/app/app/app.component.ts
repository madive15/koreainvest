...
export class AppComponent {
    ...
    router.events.subscribe((event: Event) => {
      ...
      switch (true) {
        ...
        case event instanceof NavigationStart: {
          console.log(
            'NavigationStart NavigationStart NavigationStart NavigationStart NavigationStart NavigationStart'
          );
        ...
        case event instanceof NavigationError: {
          console.log(
            'NavigationEnd NavigationEnd NavigationEnd NavigationEnd NavigationEnd NavigationEnd'
          );
          ...
          this.url = e.url;
          if (url[1]) {
            this.isNoFrame = this.isActivationPath
              ? this.isActivationPath
              : this.noFramePages.includes(url[1]); // 상하단 프레인이 없는 페이지
      ...
      .subscribe((res) => {
        ...
        if (this.isFirst) {
          if (this.lang != this.settings.language) {
            ...
            this.store.dispatch(
              SettingActions.language({ language: this.lang })
            );
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    if (event.previousIndex === this.activeNum)
      this.activeNum = event.currentIndex;
  }
  updatePageClass() {
    const currentUrl = this.router.url;
    this.isMainPage = currentUrl === '/main';
    // this.isSubPage = currentUrl === '';
  }

  getClassForPage() {
    const currentUrl = this.router.url;
    if (currentUrl === '/main') {
      return 'main-bg'; // main 페이지일 때 적용될 클래스
    } else {
      return 'sub-bg'; // sub 페이지일 때 적용될 클래스
    }
    // return ''; // 기본 클래스
  }
}
